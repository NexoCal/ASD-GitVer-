...
public class Tree<E> {
    ...
    public void DrawFrom(Node<E> node, int depth){
        for(int i=0;i<depth;i++){
            System.out.print("--");
        }
        System.out.println(" " + node.getData());
        for(int i=0;i<node.getNumChild();i++){
            Node<E> child = (Node<E>) node.getChildren().get(i);
            DrawFrom(child, depth+1);
        }
    }

    public void showChild(Node<E> node){

        for(int i = 0 ; i<node.getChildren().size(); i++){
            Node temp = (Node) node.getChildren().get(i);
            boolean Recrusion = temp.isLeaf(); 
            if (Recrusion){
